...
def find_device_by_name(device_name, selection_strategy='best'):
    ...
    if device_name.isdigit():
        ...
        try:
            ...
            print(f"[DeviceManager] Device ID {device_id} not found: {e}")
            return None, None
    ...
    if device_name.lower() in DEVICE_CONFIG:
        ...
        try:
            ...
            print(f"[DeviceManager] Configured device '{device_name}' (ID={device_id}) not found: {e}")
            return None, None
    ...
    try:
        ...
        if not matching_devices:
            ...
            if target_samplerate:
                print(f"[DeviceManager] No device found matching '{search_name}' with samplerate {target_samplerate} Hz.")
            else:
                print(f"[DeviceManager] No device found matching '{device_name}'. Available devices:")
            list_all_devices()
            return None, None
        ...
        if selection_strategy == 'first':
            ...
            if len(matching_devices) > 1:
                print(f"[DeviceManager] Found {len(matching_devices)} devices matching '{device_name}', selected first (ID={idx})")
            return idx, device
        ...
        if selection_strategy == 'best' or selection_strategy is None:
            ...
            if len(matching_devices) > 1:
                print(f"[DeviceManager] Found {len(matching_devices)} devices matching '{device_name}':")
                # 按评分排序后显示
                ...
                for dev_id, dev_info in sorted_devices:
                    ...
                    print(f"  {marker} [ID {dev_id:2d}] samplerate {int(dev_info['default_samplerate']):5d} Hz, {channels}ch - {dev_info['name'][:50]}")
        ...
        print(f"[DeviceManager] Error finding device: {e}")
        return None, None
...
def config_device(nickname, device_id):
    ...
    try:
        ...
        print(f"[DeviceManager] Configured: '{nickname}' -> ID {device_id} ({device_info['name']})")
        return True
    except Exception as e:
        print(f"[DeviceManager] Device config failed: {e}")
        return False

def print_device_config():
    """打印当前的设备配置"""
    if not DEVICE_CONFIG:
        print("[DeviceManager] No device nicknames configured")
        return
    
    print("\n=== Device Nickname Config ===")
    for nickname, device_id in DEVICE_CONFIG.items():
        try:
            device_info = sd.query_devices(device_id)
            print(f"'{nickname}' -> ID {device_id} ({device_info['name']})")
        except:
            print(f"'{nickname}' -> ID {device_id} (device unavailable)")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        print("  python device_manager.py find <device_name>  # Find device")
        print("  python device_manager.py config <nick> <ID>  # Configure nickname")
        print()
        list_all_devices()
        sys.exit(0)
    ...
    if command == 'list':
        ...
        if device_id is not None:
            print(f"[OK] Device found: ID={device_id}, name={device_info['name']}")
        else:
            print(f"[FAIL] Device not found: {device_name}")
