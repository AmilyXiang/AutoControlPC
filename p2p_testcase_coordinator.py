...
class P2PTestcaseCoordinator:
    ...
    def connect_all(self):
        ...
        print("[P2P-COORD] Connecting to PC-1...")
        if not self.pc1.connect():
            return False
        
        print("[P2P-COORD] Connecting to PC-2...")
        if not self.pc2.connect():
            self.pc1.disconnect()
            return False
        
        print("[P2P-COORD] Connected to both PCs")
        return True
    ...
    def run_simultaneous(self, pc1_xml, pc1_testcase, pc2_xml, pc2_testcase):
        ...
        print(f"\n[P2P-COORD] Execution completed")
        print(f"[P2P-COORD] PC-1 result: {self.results['pc1']}")
        print(f"[P2P-COORD] PC-2 result: {self.results['pc2']}")
...
def main():
    if len(sys.argv) < 9:
        print("Usage: python p2p_testcase_coordinator.py <pc1_host> <pc1_port> <pc2_host> <pc2_port> <pc1_xml> <pc1_testcase> <pc2_xml> <pc2_testcase>")
        print("\nExample (Call and Answer concurrent execution):")
        print("  python p2p_testcase_coordinator.py 127.0.0.1 9999 192.168.1.101 9999 \\")
        print("    testcase/p2p_network_demo.xml \"Call Flow\" \\")
        print("    testcase/p2p_network_demo.xml \"Answer Flow\"")
        sys.exit(1)
    ...
    if not coordinator.connect_all():
        print("[P2P-COORD] Connection failed")
        sys.exit(1)
